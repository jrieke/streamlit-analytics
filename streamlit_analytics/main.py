...
def stop_tracking(save_to_json: Union[str, Path] = None, verbose: bool = False):
    """
    ...
    if "analytics" in query_params and "on" in query_params["analytics"]:
        ...
        st.markdown(
            """
            Psst! 👀 You found a secret section generated by streamlit-analytics. If you 
            didn't mean to go here, remove `?analytics=on` from the URL.
...
@contextmanager
def track(save_to_json: Union[str, Path] = None, verbose=False):
    """
    ...
    yield
    stop_tracking(save_to_json=save_to_json, verbose=verbose)
